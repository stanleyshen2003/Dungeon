...
void Dungeon::createMap() {
	...
	Object* monster = new Monster("goblin",50,10,10);
	...
	Object* monster1 = new Monster("vampire Zaff",50,12,12);
	...
	num2 = rand() % 10 + 35;
	int num3 = rand() % 10 + 35;
	Object* monster2 = new Monster("Brarzamuth the Devil",200,40,40);
	...
	Object* monster3 = new Monster("Devil guardian", 700,140,140);
	...
	num2 = rand() % 10 + 35;
	num3 = rand() % 10 + 35;
	Object* monster4 = new Monster("Xuzgulun the Devil",200,40,40);
	...
	Object* monster5 = new Monster("King of Devil guardian", 999999, 99999, 99999);
...
void Dungeon::chooseAction(vector<Object*> objects) {
	...
	if(objects.size() != 0) {
        if(objects[0]->getTag() == "npc"){
            cout<<"4 : Buy something from NPC"<<endl;
            isNPC = true;
        }
	}
	int trigger = 0;
            ...
            while(cin>>useOrNot){
                if(useOrNot=="y"){
                    ...
                    while(cin>>whichToUse){
                        if(whichToUse.length()==1 && (int)whichToUse[0]-(int)'0'<=temp.size() && (int)whichToUse[0]!='0')
                            break;
                    }
	...
	for (int i = 0;i < objects.size();i++) {
        ...
        if(objects[i]->getTag()=="npc" && this->came==false){
            ...
            if(objects[i]->getName()=="god"){
                objects[i]->setScript("god : It seems that we met before...\ngod : AhA! this is where you get started XD");
            }
            this->came = true;
            continue;
        }
		...
		// battle part //
			...
			if (choice == 0) {
				...
				cout<<"you successfully retrieved..."<<endl;
				return;
			}
			else {
                vector <Item> tempInven = player.getInventory();
                for(int j=0;j<tempInven.size();j++){
